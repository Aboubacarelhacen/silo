...
export function SiloGauge({ level, status }: SiloGaugeProps) {
  const getColor = () => {
    switch (status) {
      case 'critical':
        return { fill: 'from-red-500 to-red-600', glow: 'shadow-red-500/50', text: 'text-red-600 dark:text-red-500' };
      case 'low':
        return { fill: 'from-yellow-500 to-yellow-600', glow: 'shadow-yellow-500/50', text: 'text-yellow-600 dark:text-yellow-500' };
      case 'normal':
        return { fill: 'from-teal-500 to-blue-500', glow: 'shadow-teal-500/50', text: 'text-teal-600 dark:text-teal-500' };
    }
  };
    ...
    <div className="flex flex-col items-center gap-6">
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-2">
            {[100, 75, 50, 25].map((mark) => (
              <div key={mark} className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500 dark:text-gray-600">{mark}%</span>
                <div className="h-px w-2 bg-gray-400 dark:bg-gray-700" />
              </div>
            ))}
          </div>
      ...
      <div className="text-center">
        <div className="flex items-center justify-center gap-3">
          ...
          <div>
            <div className="text-5xl text-gray-900 dark:text-white tabular-nums">{level.toFixed(1)}</div>
            <div className="text-gray-600 dark:text-gray-400">Doluluk Seviyesi</div>
          </div>
        </div>
      </div>
    </div>
